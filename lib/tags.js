...
var tags = {
  ...
  /**
   * Process the hbs_data to be used in the Handle Bar template in hbs_example.
   *
   * @example
      /**
       * Various usages:
       *
       * @hbs_data My Single Item
       *   -or-
       * @hbs_data My First Item|My Second Item
       *   -or-
       * @hbs_data My First Item
       * @hbs_data My Second Item
       *   -or-
       * @hbs_data my_class_name::My Friendly Name
       * @hbs_data my_class_name2::My Friendly Name 
       * 
       *\/
   */
  hbs_data: function() {
      ...
      // see if we already have an array. If not, get the show started
      this.block.hbs_data = this.block.hbs_data || []

      // The tag can be a single string or define multiple times to create an array.  This processes both methods.     
      // split piped strings into array items to push
      if(description.indexOf('|') > -1) {
        ...
        for (var item of description) {
            ...
            // create child arrays you can reference with {{this.[n]}} in the template
            // the double colon :: is to avoid common sentence characters like ","
            if(item.indexOf('::') > -1){
                item = item.split("::")
            }

            this.block.hbs_data.push(item); 

         }

      //create child arrays if there is a double colon.
      } else if( description.indexOf('::') > -1) {
        
        description = description.split("::")
        
        this.block.hbs_data.push(description); 
       

      // add on a plain string to the array
      } else {
         this.block.hbs_data.push(description); 
      }
  },
  ...
  /**
   * Supply a simple Handlebars Tamplate to interate over the hbs_data.
   *
   * @example
      /**
       * Simple template for H1 - H6
       *
       * @hbs_data 1|2|3|4|5|6
       *
       * @hbs_example
       * {{#each hbs_data}}
       * <h{{this}} class="d{{this}}">Display Heading H{{this}} </h{{this}}>
       * {{/each}}
       * 
       *\/
   * @example
      /**
       * More complex template for freindlier classnames
       *
       * @hbs_data text-left::Left Text
       * @hbs_data text-right::Right Text
       * @hbs_data text-center::Centered Text
       * @hbs_data text-justify::Justified Text Lorem ipsum dolor sit amet consectetur adipisicing elit. Maxime accusantium dolorem consectetur dicta eum illum velit iure ducimus aliquam sint suscipit temporibus laborum blanditiis quo est soluta beatae laudantium cumque!
       * @hbs_data text-nowrap::No Wrap Text Lorem ipsum dolor sit amet consectetur adipisicing elit. Maxime accusantium dolorem consectetur dicta eum illum.
       * @hbs_data text-wrap::Wrapped Text Lorem ipsum dolor sit amet consectetur adipisicing elit. Maxime accusantium dolorem consectetur dicta eum illum velit iure ducimus aliquam sint suscipit temporibus laborum blanditiis quo est soluta beatae laudantium cumque!
       *
       * @hbs_example
       * <ul> 
       * {{#each hbs_data}}
       * <li class="{{this.[0]}}">{{this.[1]}}</li>
       * {{/each}}
       * </ul>
       * 
       *\/
  ...
  hbs_example: function() {
      var description = this.tag.description.trim();
      var template = Handlebars.compile(description)(this.block);    
      
      // Pass the compiled template on to the example tag.
      this.block.example = {
