...
var tags = {
  ...
  section: function (params) {
    ...
    if (!params.tag.description) {
      ...
      description = params.comment.description.substr(name.length).replace(/^[\n\r]+/, '');
    }
  ...
  sectionof: function(params) {
    if (!params.tag.description) {
      throw new SyntaxError('@sectionof must reference a section (' + params.file + ':' + (params.tag.line+1) + ')');
    }
