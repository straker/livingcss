...
function parseComments(data, file, tags, context, callback) {
  ...
  comments.forEach(function(comment) {
    ...
    // default the block to be part of the index page only if it doesn't have a
    // parent section
    if (!block.page && !block.parent) {
      block.page = 'index';
      context.pages.index = context.pages.index || [];
      context.pages.index.push(block);
    }
